...
const DICTIONARIES: Record<Locale, Record<string, string>> = {
  "zh-CN": {
    ...
    "apiSettings.savedSummary": "已保存 · {provider} · {mode} · {model} · 并发 {concurrency}",
  ...
  en: {
    ...
    "apiSettings.savedSummary": "Saved · {provider} · {mode} · {model} · concurrency {concurrency}",
...
function languageSwitcherElement(): HTMLElement | null {
  try {
    return getLegacyBridge().els.languageSwitcher as HTMLElement | null | undefined || null;
  } catch {
    return null;
  }
}
...
function updateLanguageSwitcher(): void {
  ...
  switcher?.querySelectorAll<HTMLElement>("[data-language-option]").forEach((button) => {
...
export function restoreLocalePreference(): void {
  let saved: string = DEFAULT_LOCALE;
  try {
...
function bindLanguageSwitcher(): void {
  ...
  switcher?.addEventListener("click", (event) => {
    ...
    const button = target.closest<HTMLElement>("[data-language-option]");
    if (!button) return;
