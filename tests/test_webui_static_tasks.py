...
class WebUIStaticTaskTests(WebUIStaticTestCase):
    ...
    def test_history_page_static_contract_exists(self) -> None:
        ...
        self.assertIn('/static/history.js?v=history-33', history_html)
    ...
    def test_preview_outputs_can_be_collected_into_floating_reference_bar(self) -> None:
        ...
        self.assertIn('formatTranslation("referenceCollector.stagedAction"', script)
        ...
        self.assertIn("function pendingReferenceSource", script)
        self.assertIn("async function addPreviewReferenceToInput", script)
        self.assertIn('state.pendingReferenceAdds = [...(state.pendingReferenceAdds || []), { id, url: normalizedUrl }]', script)
        self.assertIn('translate("referenceCollector.addingDuplicate")', script)
        self.assertIn('translate("preview.addingReference")', script)
        self.assertIn('classList.add("is-pending")', script)
        self.assertIn('function pendingReferenceInputs()', script)
        ...
        self.assertRegex(styles, r"\.pending-reference-thumb\s*\{[^}]*opacity:")
        self.assertRegex(styles, r"\.add-to-input-btn:disabled\s*,\s*\.add-to-input-btn\.is-pending\s*\{[^}]*cursor:\s*progress")
        self.assertRegex(styles, r"\.reference-collector\s*\{[^}]*bottom:\s*10px")
        self.assertRegex(styles, r"\.reference-collector\s*\{[^}]*left:\s*12px")
    ...
    def test_user_guide_drawer_is_available_from_top_navigation(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn('id="userGuideButton"', html)
        self.assertIn('id="userGuideDrawer"', html)
        self.assertIn('id="userGuideSearch"', html)
        self.assertIn('id="userGuideNav"', html)
        self.assertIn('id="userGuideBody"', html)
        self.assertIn('data-i18n="userGuide.open"', html)
        self.assertIn("import { initUserGuideFeature } from \"./user-guide\"", Path("codex_image/webui/frontend/src/main.ts").read_text(encoding="utf-8"))
        self.assertIn("export function initUserGuideFeature", script)
        self.assertIn("const GUIDE_SECTIONS", script)
        self.assertIn("userGuide.quickStart", script)
        self.assertIn("userGuide.gallery", script)
        self.assertIn("userGuide.templates", script)
        self.assertIn("userGuide.faq", script)
        self.assertIn("userGuideButton: document.querySelector(\"#userGuideButton\")", script)
        self.assertIn("#userGuideDrawer.open", script)
        self.assertRegex(styles, r"\.user-guide-button\s*\{[^}]*gap:")
        self.assertRegex(styles, r"\.user-guide-drawer\s*\{[^}]*position:\s*fixed")
        self.assertRegex(styles, r"\.user-guide-drawer\.open\s*\{[^}]*transform:\s*translateX\(0\)")
