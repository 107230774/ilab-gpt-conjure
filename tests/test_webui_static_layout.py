...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    def test_task_notification_settings_are_in_storage_modal(self) -> None:
    ...
    def test_gallery_categories_are_managed_from_server_in_management_sheet(self) -> None:
        ...
        self.assertRegex(styles, r"\.gallery-grid-layer\s*\{[^}]*grid-template-columns")
    ...
    def test_auth_source_switcher_is_available_in_top_nav(self) -> None:
        ...
        self.assertIn('aria-label="API 设置"', html)
        self.assertIn('title="API 设置"', html)
    ...
    def test_api_source_switcher_and_settings_modal_exist(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")
        ...
        self.assertIn('data-auth-source="api"', html)
        self.assertNotIn('onclick="setAuthSource', html)
        self.assertIn('id="apiSettingsModal"', html)
        ...
        self.assertIn('class="api-settings-feedback"', html)
        ...
        self.assertIn("apiSettingsModal: document.querySelector", script)
    ...
    def test_primary_panel_close_buttons_use_consistent_x_button(self) -> None:
        ...
        expected_buttons = {
            ...
            "settingsModalClose": "关闭存储设置面板",
            "apiSettingsModalClose": "关闭 API 设置面板",
    ...
    def test_dashboard_uses_two_column_workspace(self) -> None:
        ...
        self.assertNotIn("json-panel", html)
    ...
    def test_sidebar_settings_modal_can_manage_storage_paths(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")
        ...
        self.assertIn("settingsButton: document.querySelector(\"#settingsButton\")", script)
        self.assertIn('fetch("/api/settings")', script)
        self.assertIn('fetch("/api/settings", {', script)
        self.assertIn("restart_required", script)
        self.assertIn("重启 WebUI 后生效", script)
        self.assertRegex(styles, r"\.settings-modal-panel\s*\{[^}]*width:\s*min\(620px,\s*94vw\)")
    ...
    def test_input_image_editor_javascript_contract_exists(self) -> None:
        ...
        self.assertIn('if (!["brush", "arrow", "crop", "fill"].includes(tool)) return;', script)
        ...
        self.assertIn("const shaftEnd = {", script)
        self.assertIn("ctx.lineTo(shaftEnd.x, shaftEnd.y);", script)
