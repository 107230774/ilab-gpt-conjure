...
class WebUIStaticPromptTests(WebUIStaticTestCase):
    ...
    def test_prompt_editor_supports_hex_color_code_picker(self) -> None:
        ...
        self.assertRegex(styles, r"\.color-suggest-actions\s*\{[^}]*grid-template-columns:\s*calc\(4em \+ 18px\) auto auto auto auto")
        self.assertRegex(styles, r"\.color-suggest-actions\s*\{[^}]*overflow-x:\s*auto")
        self.assertRegex(styles, r"\.color-name-input\s*\{[^}]*width:\s*calc\(4em \+ 18px\)")
        self.assertNotRegex(styles, r"\.color-name-input\s*\{[^}]*flex:\s*1 1 120px")
...
"""
    ...
    def test_preview_shows_prompt_popover_for_each_output(self) -> None:
        ...
        self.assertRegex(styles, r"\.add-to-input-btn\s*,\s*\.collect-input-btn\s*,\s*\.prompt-popover-button\s*,\s*\.preview-download-link\s*\{[^}]*background:\s*color-mix\(in srgb,\s*var\(--surface\) 94%,\s*transparent\)")
